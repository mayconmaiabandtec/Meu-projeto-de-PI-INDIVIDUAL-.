...
function iniciar_escuta() {
    ...
    porta_serial.list().then(entradas_seriais => {
        ...
        // console.log('entradas_seriais: ', entradas_seriais);
        var entradas_seriais_arduino = entradas_seriais.filter(entrada_serial => {
            // console.log(entrada_serial.vendorId,'vendorid' , entrada_serial.productId,'productid');
            return entrada_serial.vendorId == '2341' && entrada_serial.productId == '0042';
        });
        ...
        parser.on('data', (dados) => {
            ...
            try {
                ...
                registrar_leitura(parseFloat(leitura[0]), parseFloat(leitura[1]));
            } catch (e) {
...
function registrar_leitura(temperatura, umidade) {
    ...
    banco.conectar().then(() => {
        ...
        return banco.sql.query(`INSERT into sensor (temperatura,umidade,datahora)
                                values 
                                ('${temperatura}','${umidade}',CONVERT(Datetime, '${agora()}', 120))`);
    }).catch(erro => {
